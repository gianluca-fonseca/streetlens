...
- **Glass is true backdrop-blur, over live map tiles ONLY.** `--glass-bg` is a neutral white tint (`rgba(255,255,255,0.66)` light / `rgba(10,10,10,0.58)` dark), `--glass-border` a neutral hairline, and `--glass-blur` (20px) is the sanctioned blur radius for over-tile panels, with `--glass-blur-chip` (14px) for the smaller over-tile chips/controls. Never on flat grounds.
